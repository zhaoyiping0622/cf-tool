...
func getLatest() (version, note, ptime, url string, size uint, err error) {
	...
	resp, err := http.Get("https://api.github.com/repos/zhaoyiping0622/cf-tool/releases/latest")
	if err != nil {
		return
	}
	...
	url = fmt.Sprintf("https://github.com/zhaoyiping0622/cf-tool/releases/download/%v/cf_%v_%v_%v.zip", version, version, goos, arch)
